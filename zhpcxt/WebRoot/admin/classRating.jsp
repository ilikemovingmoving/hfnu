...
<%
    ...
    String classPro =  request.getParameter("classPro");
...
<head>
    ...
    <script type="text/javascript">
        ...
        function getPro(pro) {
            document.classSelect.action = "<%=basePath%>admin/classRating.jsp?classPro="
            + pro;
            document.classSelect.submit();

        }
        function getGrade(grade) {
            document.classSelect.action = "<%=basePath%>admin/classRating.jsp?classGrade="
            + grade;
            document.classSelect.submit();

        }
        /* function getPro_Grade(){
        	var classGrade=$("select[name='classGrade']").val();
        	var classPro=$("select[name='classPro']").val();
        	document.classSelect.action = "/zhpcxt/admin/classRating.jsp?classGrade="
        			+classGrade+"&classPro="+classPro;
        	document.classSelect.submit();
            ...
            <tr class="thead">
                ...
                <td align="center">
                    <select name="classPro" onchange="getPro(this.value);">
                        <option value="" style="text-align:center">专业</option>
                        <%
            	...
            	if(res!=null){
            		...
            		if(list!=null && list.size()>0){
            			...
            			for(Object o:list){
            				...
            				out.println("<td align='center'>"+StringUtils.trimToEmpty(item.get("class_name")+"")+"</td>");
            				out.println("<td align='center'>"+StringUtils.trimToEmpty(item.get("mgr")+"")+"</td>");
            				out.println("<td align='center'><a style='text-decoration:underline;' href='/zhpcxt/secretary/ratingSco.jsp?class_id="+StringUtils.trimToEmpty(item.get("class_id")+"")+"'>查看测评分数</a></td>");
            				out.println("</tr>");
            			}
            		}
            	}
